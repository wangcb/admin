...
layui.define(['table'], function (exports) {
    var setter = {
        ...
        ajaxSuccessBefore: function (res, url, obj) {
            var operate_time = layui.data(setter.tableName);
            var now = parseInt(new Date().getTime() / 1000);
            if (operate_time.hasOwnProperty('timestamp') && now - operate_time.timestamp > 7200) {
                setter.removeToken();
                location.replace('/login/login.html');
            }
            layui.data(setter.tableName, {key:"timestamp",value:now});
            if (res.code === 1004) {  // 登录过期退出到登录界面
                setter.removeToken();
